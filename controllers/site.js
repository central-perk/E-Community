...
exports.index = function(req, res, next) {
  ...
  //用户的话题
  cache.get('user_topics', proxy.done(function(user_topics) {
  ...
  console.log('./all');
  proxy.all('topics', 'no_reply_topics', 'pages', 'user_topics',
    ...
    if (sitemapData) {
      ...
      Topic.getLimit5w(function(err, topics) {
        ...
        topics.forEach(function(topic) {
          urlset.ele('url').ele('loc', 'https://club.echuandan.com/topic/' + topic._id);
        });
...
exports.u = function(req, res) {
  if (req.user) {
    return res.redirect(url.resolve(config.hosts.home, path.join('u', req.user._id.toString())));
  } else {
    return redirectSignin(res);
  }
}
