...
var Topic = require('../proxy')
	.Topic;
var TopicCollect = require('../proxy')
	.TopicCollect;
...
exports.index = function (req, res, next) {
	...
	var ep = EventProxy.create(events, function (topic, other_topics, user_topics) {
	...
	Topic.getFullTopic(topic_id, ep.done(function (message, topic, author, replies) {
		...
		topic.reply_up_threshold = (function () {
			...
			allUpCount = _.sortBy(allUpCount, Number)
				.reverse();
		...
		if (!req.user) {
			...
			TopicCollect.getTopicCollect(req.user._id, topic._id, ep.done(function (doc) {
...
exports.create = function (req, res, next) {
	...
	Topic.getTopicsByQuery({
		author_id: user._id
	}, options2, function (err, user_topics) {
...
exports.showEdit = function (req, res, next) {
	...
	Topic.getTopicById(topic_id, function (err, topic, tags) {
		...
		Topic.getTopicsByQuery({
			...
			if (String(topic.author_id) === String(req.user._id) || req.user.is_admin) {
				res.render('topic/edit', {
					action: 'edit',
					topic_id: topic._id,
					title: topic.title,
